...
- In the [supervised]{style="color: #0000FF;"} learning [regression]{style="color: #B069DB;"}, we observe the values of a [quantitative]{style="color: #B069DB;"} [response]{style="color: #0000FF;"} $Y$, as well as $p$ many predictors $X_1, \dots, X_p$ (these may be quantitative or categorical)
...
**Q:** What could go wrong if we try to minimize $\mathrm{MSE}$ on observed data?
  ...
  - Hint: Consider prediction with new data
...
data.frame(
  ...
  MSE = round(c(lin_mse_train, quintic_mse_train, poly_mse_train), 3)
)
```
...
data.frame(
  model = c("Linear", "Quintic", "Poly"),
  MSE = round(c(lin_mse, quintic_mse, poly_mse), 3)
)
```
