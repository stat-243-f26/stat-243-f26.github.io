...
lin_pred <-predict(lin_mod, adelie_train)

# Training polynomial model
poly_mod <- lm(body_mass_g ~ poly(bill_length_mm, degree = 5), data = adelie_train)
poly_pred <- predict(poly_mod, adelie_train)

# MSE linear model
mean((adelie_train$body_mass_g - lin_pred)^2)
# MSE polynomial model
mean((adelie_train$body_mass_g - poly_pred)^2)
```
...
#| fig-width: 6
#| out-width: "65%"
train_test <- adelie_train %>%
  mutate(dataset = "training") %>%
  bind_rows(adelie_test %>% mutate(dataset = "testing"))
ggplot() + 
  geom_point(data = adelie_train, 
             aes(x = bill_length_mm, y = body_mass_g)) +
    geom_point(data = train_test, 
             aes(x = bill_length_mm, y = body_mass_g, color = dataset)) +
  scale_color_manual(values = c("magenta3", "black")) + 
  theme_bw(base_size=15) + 
  labs(title="Testing Data",x="Bill Length",y="Body Mass") + 
...
#| message: false
#| echo: false
ggplot(adelie_test,aes(x=bill_length_mm,y=body_mass_g))+geom_point(color = "magenta3") +
